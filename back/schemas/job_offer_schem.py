...
class JobOfferUpdate(SQLModel):
    job_ref: Optional[str] = Field(..., max_length=12, description="Unique reference for the job offer")
    title: Optional[str] = Field(..., max_length=50, description="Title of the job offer")
    job_location: Optional[str] = Field(..., max_length=100, description="Location of the job")
    salary: Optional[Decimal] = Field(..., gt=0, description="Salary for the job offer, must be greater than zero")
    description: Optional[str] = Field(..., description="Detailed description of the job offer")
    company_description: Optional[str] = Field(..., description="Description of the company")
    posted_date: Optional[datetime.date] = Field(default_factory=datetime.date.today, description="Date the job was posted")
    end_of_application: Optional[datetime.date] = Field(..., description="Deadline for applications")
    id_recruiter: Optional[int] = Field(..., description="ID of the recruiter posting the job offer")
    skill_ids: Optional[List[int]] = Field(default=None, description="List of skill IDs required for the job")
    soft_skill_ids: Optional[List[int]] = Field(default=None, description="List of soft skill IDs required for the job")
